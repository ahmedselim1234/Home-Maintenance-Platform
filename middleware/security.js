...
const csrfProtection = (req, res, next) => {
  ...
  const tokenFromHeader = req.headers["csrf-token"] || req.body._csrf;

  console.log("secret", req.cookies.csrf_secret);
  console.log("token", req.headers["csrf_token"]);

  if (["POST", "PUT", "DELETE", "PATCH"].includes(req.method)) {
    if (
      !secret ||
      !tokenFromHeader ||
      !tokens.verify(secret, tokenFromHeader)
    ) {
